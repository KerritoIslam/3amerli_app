...
class PaymentsRemoteDataSource {
  ...
  Future<TransactionModel> createTransaction(
    ...
    try {
      developer.log('createTransaction payload: $payload',
      ...
      if (isSuccess(resp.statusCode) && resp.data != null) {
        ...
        developer.log('createTransaction response: $data',
            name: 'PaymentsRemoteDataSource');
        ...
        return TransactionModel.fromJson({
          'id': -1,
          'userId': payload['buyerId'] ?? 0,
          'amount': payload['totalAmount'] ?? payload['amount'] ?? 0.0,
          'status': 'PENDING',
      ...
      developer.log(
          'Dio error createTransaction - status: $status, serverResponse: $serverResp',
          name: 'PaymentsRemoteDataSource',
          error: e,
      ...
      developer.log('Unexpected error createTransaction: $e',
          name: 'PaymentsRemoteDataSource',
          error: e,
          stackTrace: st as StackTrace?);
      throw ApiException('Unexpected error while creating transaction');
    }
  }
  ...
  Future<List<PaymentCardModel>> fetchCards(
      {int page = 1, int pageSize = 50}) async {
    await Future.delayed(const Duration(seconds: 2));
    final sample = '''[
