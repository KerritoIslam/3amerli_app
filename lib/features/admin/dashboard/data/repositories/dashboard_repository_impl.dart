...
class DashboardRepositoryImpl implements DashboardRepository {
  ...
  Future<DashboardStats> getDashboardStats() async {
    ...
    try {
      ...
      final dailyResp = await apiService
          .get('/analytics/daily-stats', queryParameters: {'limit': 7});
      final body = dailyResp.data;
      if (body != null) {
        // If the endpoint returns a structured summary (orders/revenue/supermarkets), extract direct fields
        if (body is Map &&
            (body.containsKey('orders') ||
                body.containsKey('revenue') ||
                body.containsKey('supermarkets') ||
                body.containsKey('deliveredOrders'))) {
          try {
            final orders = body['orders'];
            if (orders is Map) {
              final rawCount =
                  orders['count'] ?? orders['ordersCount'] ?? orders['value'];
              if (rawCount is num) {
                totalOrders = rawCount.toInt();
              } else if (rawCount is String)
                totalOrders = int.tryParse(rawCount) ?? totalOrders;

              final rawPct = orders['percentageChange'] ??
              ...
              if (rawPct is num) {
                ...
                ordersPercentageChange =
                    double.tryParse(rawPct) ?? ordersPercentageChange;
            }

            final revenue = body['revenue'];
            if (revenue is Map) {
              final rawTurn =
                  revenue['turnover'] ?? revenue['total'] ?? revenue['value'];
              if (rawTurn is num) {
                totalRevenue = rawTurn.toDouble();
              } else if (rawTurn is String)
                totalRevenue = double.tryParse(rawTurn.replaceAll(',', '')) ??
                    totalRevenue;

              final rawRevPct =
                  revenue['percentageChange'] ?? revenue['percentChange'];
              if (rawRevPct is num) {
                revenuePercentageChange = rawRevPct.toDouble();
              } else if (rawRevPct is String)
                revenuePercentageChange = double.tryParse(rawRevPct);
            }

            final supermarkets = body['supermarkets'];
            if (supermarkets is Map) {
              final rawActive = supermarkets['todayActiveSuperMarkets'] ??
                  supermarkets['active'] ??
                  supermarkets['count'];
              if (rawActive is num) {
                supermarketsActiveToday = rawActive.toInt();
              } else if (rawActive is String)
                supermarketsActiveToday =
                    int.tryParse(rawActive) ?? supermarketsActiveToday;

              final rawSuperPct = supermarkets['percentageChange'] ??
                  supermarkets['percentChange'];
              if (rawSuperPct is num) {
                supermarketsPercentageChange = rawSuperPct.toDouble();
              } else if (rawSuperPct is String)
                supermarketsPercentageChange = double.tryParse(rawSuperPct) ??
                    supermarketsPercentageChange;
            }

            final delivered = body['deliveredOrders'];
            if (delivered is Map) {
              final rawDelPct =
                  delivered['percentageChange'] ?? delivered['percentChange'];
              if (rawDelPct is num) {
                deliveredOrdersPercentageChange = rawDelPct.toDouble();
              } else if (rawDelPct is String)
                deliveredOrdersPercentageChange = double.tryParse(rawDelPct) ??
                    deliveredOrdersPercentageChange;
              final rawDelCount = delivered['count'] ?? delivered['value'];
              if (rawDelCount is num) {
                deliveredOrdersCount = rawDelCount.toInt();
              } else if (rawDelCount is String)
                deliveredOrdersCount =
                    int.tryParse(rawDelCount) ?? deliveredOrdersCount;
            }
          } catch (_) {}
        }
        ...
        if (list != null) {
          for (final e in list) {
            try {
              ...
              final date =
                  (m['date'] ?? m['label'] ?? m['day'] ?? '').toString();
              final amount = (() {
                final a = m['amount'] ?? m['turnover'] ?? m['value'] ?? 0;
                if (a is num) return a.toDouble();
                if (a is String)
                  return double.tryParse(a.replaceAll(',', '')) ?? 0.0;
                return 0.0;
              })();
              final ordersCount = (() {
              ...
              recentSales.add(DailySalesModel(
                  date: date, amount: amount, ordersCount: ordersCount));
            } catch (_) {
    ...
    try {
      ...
      if (tb != null) {
        ...
        if (tb is List) {
          ...
          list = (tb['data'] as List?) ??
              (tb['result'] as List?) ??
              (tb['items'] as List?) ??
              (tb['topProducts'] as List?);
      ...
      if (b is List) {
        try {
          ...
          for (final item in b) {
            if (item is Map) {
              ...
              final rawMonth =
                  (m['month'] ?? m['label'] ?? m['monthName'] ?? '').toString();
              final rawTurn = m['turnover'] ?? m['value'] ?? m['total'] ?? 0;
              double turn = 0;
              if (rawTurn is num) {
                turn = rawTurn.toDouble();
              } else if (rawTurn is String)
                turn = double.tryParse(rawTurn.replaceAll(',', '')) ?? 0.0;

              if (rawMonth.isNotEmpty) {
                // normalize to 3-letter english month (first 3 chars)
                final key = rawMonth.substring(
                    0, rawMonth.length >= 3 ? 3 : rawMonth.length);
          ...
          for (final dt in expectedMonths) {
            ...
            recentSales.add(DailySalesModel(
                date: dt.toIso8601String(), amount: amount, ordersCount: 0));
          }

          // set a fallback totalRevenue from the turnover list if not present
          if (totalRevenue == 0.0) {
            totalRevenue = recentSales
                .map((s) => s.amount)
        ...
        if (rawRevenue != null) {
          if (rawRevenue is num) {
            ...
            totalRevenue =
                double.tryParse(rawRevenue.replaceAll(',', '')) ?? totalRevenue;
          }
        }
        ...
        if (rawTotalOrders != null) {
          ...
          } else if (rawTotalOrders is String)
            totalOrders = int.tryParse(rawTotalOrders) ?? totalOrders;
        }
        ...
        if (rawPending != null) {
          if (rawPending is num) {
            ...
            pendingOrders = int.tryParse(rawPending) ?? pendingOrders;
        }
        ...
        if (rawCompleted != null) {
          if (rawCompleted is num) {
            ...
            completedOrders = int.tryParse(rawCompleted) ?? completedOrders;
        }
      }
    } catch (e) {
      // ignore
    ...
    try {
      ...
      if (sb is Map) {
        totalUsers =
            (sb['totalUsers'] ?? sb['usersCount'] ?? totalUsers) as int? ??
                totalUsers;
        totalProducts = (sb['totalProducts'] ??
                sb['productsCount'] ??
    ...
    if (totalOrders == 0 && recentSales.isNotEmpty) {
      totalOrders =
          recentSales.map((s) => s.ordersCount).fold<int>(0, (p, e) => p + e);
    }
