...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  void _onAdd(CartAddItemEvent event, Emitter<CartState> emit) {
    ...
    if (current is CartLoaded) {
      ...
      // ignore: avoid_print
      print(
          'CartBloc _onAdd -> items now: ${items.map((e) => '${e.productId}(${e.quantity})').join(', ')}');
    } else {
      emit(CartLoaded([event.item]));
      // debug
      // ignore: avoid_print
      print(
          'CartBloc _onAdd -> items now: ${event.item.productId}(${event.item.quantity})');
    }
  }
  ...
  void _onRemove(CartRemoveItemEvent event, Emitter<CartState> emit) {
    ...
    if (current is CartLoaded) {
      final items =
          current.items.where((i) => i.productId != event.productId).toList();
      emit(CartLoaded(items));
    }
  }
  ...
  void _onUpdate(CartUpdateQuantityEvent event, Emitter<CartState> emit) {
    ...
    if (current is CartLoaded) {
      ...
      // ignore: avoid_print
      print(
          'CartBloc _onUpdate -> items now: ${items.map((e) => '${e.productId}(${e.quantity})').join(', ')}');
    }
  }
